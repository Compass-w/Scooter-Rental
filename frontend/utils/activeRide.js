...
export const normalizeActiveRide = (ride = {}, fallback = {}) => {
  ...
  const metadata = {
    ...
    pickupStoreName: String(ride.pickupStoreName ?? fallback.pickupStoreName ?? '').trim(),
    pickupStoreAddress: String(ride.pickupStoreAddress ?? fallback.pickupStoreAddress ?? '').trim(),
    returnStoreCode: String(ride.returnStoreCode ?? fallback.returnStoreCode ?? '').trim(),
    returnStoreName: String(ride.returnStoreName ?? fallback.returnStoreName ?? '').trim(),
    returnStoreAddress: String(ride.returnStoreAddress ?? fallback.returnStoreAddress ?? '').trim(),
